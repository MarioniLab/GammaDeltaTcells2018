...
```{r, data}
data.all <- read.table("../Data/FilteredCounts.txt", sep = "\t")
...
# Read in genenames
genes <- read.table("../Data/Mouse_genes.txt", sep = "\t", header = TRUE, stringsAsFactors = FALSE)
...
for(i in 1:length(list.comp)){
  ...
  write.xlsx(cur_list, paste("../Results/Marker_genes/", names(list.comp)[i], ".xlsx", sep = ""))
  write.xlsx(cur_list.go, paste("../Results/Marker_genes/", names(list.comp)[i], "_GO.xlsx", sep = ""))
}
```